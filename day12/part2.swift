...
for i in 0..<codes.count {
    ...
    let p1 = numOfSpec(&codes[i], 0, &nums[i], &pattern) 
  
    if codes[i][codes[i].count - 1] == "1" {
        cnt += Int(pow(Double(p1),5.0))
    } else {
        codes[i].insert("?", at: 0)
        codes[i].append("?")

        
        let p2 = numOfSpec(&codes[i], 0, &nums[i], &pattern)

        let df = p1*(Int(pow(Double(p2),4.0)))     
